...
def test_gateway_verifies_unchanged_evidence_bundle(monkeypatch, tmp_path: Path):
    monkeypatch.setenv("MEMORY_CONTROLLER_HMAC_SECRET", "test-secret")
    gateway = CognitiveGateway(settings=Settings(vault_path=tmp_path, sync_vault=False), provider=None)
    gateway.vault_bridge = FakeBridge()
    bundle = gateway.acquire_conflict_evidence(memory_ids=["m1", "m2"], conflict_case_id="CR-test")
    result = gateway.verify_conflict_evidence(bundle=bundle)
