...
class ConflictReviewService:
    ...
    def issue_verdict(self, *, principal: Any, reviewer: str, verdict: str, memory_ids: Iterable[str], evidence_bundle_hash: str, evidence_valid: bool, reason: str, as_of: Any = None, known_as_of: Any = None) -> dict[str, Any]:
        ...
        return AuthorizedVerdictEngine().issue(
            principal=principal_enum, reviewer=reviewer, verdict=verdict,
            memory_ids=memory_ids, evidence_bundle_hash=evidence_bundle_hash,
            evidence_valid=evidence_valid, reason=reason,
    ...
    def apply_verdict(self, *, principal: Any, verdict: dict[str, Any], evidence_verification: dict[str, Any], review_state: dict[str, Any], action: str, reason: str) -> dict[str, Any]:
        ...
        try:
            ...
            verdict_obj = AuthorizedVerdict(
                verdict_id=str(verdict["verdict_id"]), verdict=Verdict(str(verdict["verdict"])),
                reviewer=str(verdict["reviewer"]), reviewer_principal=str(verdict["reviewer_principal"]),
                memory_ids=tuple(str(x) for x in verdict["memory_ids"]),
                evidence_bundle_hash=str(verdict["evidence_bundle_hash"]),
                as_of=verdict.get("as_of"), known_as_of=verdict.get("known_as_of"),
                evidence_valid=bool(verdict["evidence_valid"]), reason=str(verdict["reason"]),
    ...
    def promote_learning_candidate(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError("Canonical learning promotion gate is unavailable") from exc
        try:
            principal_enum = principal if isinstance(principal, Principal) else Principal(str(principal))
        except ValueError as exc:
            raise ValueError(f"Invalid reviewer principal: {principal!r}") from exc
        return LearningPromotionGate(controller).apply(
